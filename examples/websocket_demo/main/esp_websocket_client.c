...
#define WS_TRANSPORT_OPCODES_FIN 0x80
...
#define WEBSOCKET_RECONNECT_TIMEOUT_MS  (2*1000)
...
#define ESP_WS_CLIENT_MEM_CHECK(TAG, a, action) if (!(a)) {                                         \
        ESP_LOGI(TAG, "%s:%d (%s): %s", __FILE__, __LINE__, __FUNCTION__, "Memory exhausted");       \
        action;                                                                                     \
        }

#define ESP_WS_CLIENT_STATE_CHECK(TAG, a, action) if ((a->state) < WEBSOCKET_STATE_INIT) {           \
        ESP_LOGI(TAG, "%s:%d (%s): %s", __FILE__, __LINE__, __FUNCTION__, "Websocket already stop");  \
        action;                                                                                      \
        }
...
static bool ws_login = false;
void iotswitch_login(void);
...
#define WS_RECV_MAX_BYTES   160
static char ws_recv_cache[WS_RECV_MAX_BYTES] = {0};
...
static esp_err_t esp_websocket_client_dispatch_event(esp_websocket_client_handle_t client,
                                                     ...
                                                     int data_len)
{
    if (WEBSOCKET_EVENT_DATA == event) {

        if (data_len != 0) {

            memset(ws_recv_cache, 0, sizeof(ws_recv_cache));
            if (data_len > WS_RECV_MAX_BYTES) {
                data_len = WS_RECV_MAX_BYTES;
            }

            memcpy(ws_recv_cache, data, data_len);

            printf("%s\n", ws_recv_cache);
            
        } else {
            ESP_LOGI(TAG, "esp_websocket_client_dispatch_event: op_code=0x%x data_len=%d",
                    client->last_opcode,
                    data_len
                    );
        }
...
_websocket_init_fail:
    ESP_LOGI(TAG, "_websocket_init_fail");
...
static int esp_websocket_client_send_with_opcode(esp_websocket_client_handle_t client, ws_transport_opcodes_t opcode, const char *data, int len, TickType_t timeout)
{
    ...
    if (xSemaphoreTakeRecursive(client->lock, timeout) != pdPASS) {
        ESP_LOGE(TAG, "Could not lock ws-client within %ld timeout", timeout);
        return ESP_FAIL;
    }
...
static void websocket_reconnect_task(void *pv)
{
    int report_ts = 0;
    char *report_buffer;

    report_buffer = (char*)malloc(256);
    ...
    while(1) {
        ...
        if (report_ts++ >= 10) {
            if (ws_login) {
                ...
                memset(report_buffer, 0, 256);
                strcpy(report_buffer, "world");       
                printf("Sending %s\n", report_buffer);
                esp_websocket_client_send_text(client, report_buffer, strlen(report_buffer), 300);  //3秒超时时间 
                report_ts = 0;
            }
        }
    }
...
void websocket_reconnect_app(void) {
    xTaskCreate(websocket_reconnect_task, "websocket_reconnect", 4096, NULL, 12, NULL);
}
